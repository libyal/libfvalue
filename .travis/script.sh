...
then
	docker run -t -v "${PWD}:/libfvalue" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libfvalue && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libfvalue;
