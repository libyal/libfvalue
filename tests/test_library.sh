...
LIBRARY_TESTS="binary_data data_handle error floating_point integer split_utf16_string split_utf8_string string support table value value_entry";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./fvalue_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfvalue" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
